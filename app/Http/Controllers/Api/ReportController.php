...
class ReportController extends Controller
{
  ...
  public function depositReportChartDepartment(Request $request)
  {
    // $facultyData = array();
    // $facultyColor = array();
    // $facultyDataTotal = array();
    ...
    foreach ($department as $key => $value) {
      $theFaculty = strtolower($value->department_name);
      // dd($theFaculty);
      ...
      for ($i = 0; $i < count($uid); $i++) {
        $usr = User::find($uid[$i]->user_id);
        array_push($arr, $usr->department);
      }

      $allDataBydepartment = array_count_values($arr);
      $val_id = (array_key_exists($value->id, $allDataBydepartment)) ? $allDataBydepartment[$value->id] : 0;
    ...
    switch ($request->filter) {
      ...
      case 'department':
        $uploadForm = UploadForm::with(
          ...
          ->whereHas('uploadFormDepartment', function ($query) use ($request) {
            $query->where('department', $request->department);
          })
          ->with("uploadFormDepartment")
          ->orderBy($request->sort, $request->order)
          ->get();
        break;
      ...
      default:
        ...
        if($request->has('report_name') && $request->report_name != '' ){

           if($request->has('sort') && $request->sort == 'id'){
              $uploadForm = $uploadForm->where('id', $request->report_name);
           }

            if($request->has('sort') && $request->sort == 'title'){
                $uploadForm = $uploadForm->where('title', $request->report_name);
            }

            if($request->has('sort') && $request->sort == 'category'){
                $uploadForm = $uploadForm->where('category', $request->report_name);
            }

            if($request->has('sort') && $request->sort == 'date'){
                $uploadForm = $uploadForm->where('date', $request->report_name);
            }

            if($request->has('sort') && $request->sort == 'status'){
                $uploadForm = $uploadForm->where('status', $request->report_name);
            }

            if($request->has('sort') && $request->sort == 'department'){
                $uploadForm = $uploadForm->whereHas('uploadFormDepartment', function ($query) use ($request) {
                  $query->where('department', $request->report_name);
                });
            }
