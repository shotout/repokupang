...
const getters = {
  ...
  getAllContributors: (state) => state.contributors,
...
const actions = {
  ...
  getContributors({ commit }) {
    axios.get("/api/contributors").then((response) => {
      console.log(response.data.data.contributors);
      commit("SET_CONTRIBUTORS", response.data.data.contributors);
    });
  },
